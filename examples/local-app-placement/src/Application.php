...
class Application
{
    ...
    public static function processRequest(Request $incomingRequest): Response
    {
        ...
        try {
            ...
            if (PlacementRequest::isCanProcess($incomingRequest)) {
                ...
                self::getLog()->debug('processRequest.defaultPlacement');
                // todo process other placement request
                var_dump('hello from application');
            ...
            self::getLog()->error('processRequest.error', [
                'message' => $baseException->getMessage(),
                'type' => $baseException::class,
                'trace' => $baseException->getTraceAsString()
            ]);
            return new Response(sprintf('internal error: %s', $baseException->getMessage()), 200);
        }
    }
    ...
    protected static function processOnInstallPlacementRequest(PlacementRequest $placementRequest): void
    {
        ...
        $hostUrl = $placementRequest->getRequest()->server->get('HTTP_HOST');
        ...
        $currentB24UserId = self::getB24Service($placementRequest->getRequest())
            ...
            ->getUserProfile()
            ->ID;
        $eventHandlerUrl = sprintf('https://%s/event-handler.php', $hostUrl);
        ...
        // register placements
        self::getB24Service($placementRequest->getRequest())
            // unbind if reinstall
            ->getPlacementScope()->placement()->unbind(PlacementLocationCode::CRM_DEAL_DETAIL_TAB);
        // bind placement handler
        self::getB24Service($placementRequest->getRequest())
            ->getPlacementScope()->placement()->bind(
                PlacementLocationCode::CRM_DEAL_DETAIL_TAB,
                sprintf('https://%s/placements/deal-detail/index.php', $hostUrl),
                [
                    'RU' => [
                        'TITLE' => 'app title',
        ...
        self::getLog()->debug('processRequest.processOnInstallPlacementRequest.finishBindPlacements');
