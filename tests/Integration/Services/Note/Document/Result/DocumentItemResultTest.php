...
use Bitrix24\SDK\Services\Note\Document\Result\DocumentItemResult;
...
class DocumentItemResultTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->collectionId = $this->collectionService->add('SDK document annotation test ' . time())
            ->collection()->id;
        $this->documentId = $this->documentService->add($this->collectionId, 'Annotation test document', null, '# Hi')
            ->document()->id;
    }
    ...
    {
        $rawItem = $this->documentService->get($this->documentId)
    ...
    {
        $document = $this->documentService->get($this->documentId)->document();
        $this->assertBitrix24ResultItemFieldsTypeCastMatchAnnotations($document, DocumentItemResult::class);
    }
}
