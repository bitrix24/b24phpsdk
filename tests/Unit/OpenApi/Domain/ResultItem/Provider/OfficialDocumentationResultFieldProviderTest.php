...
use Bitrix24\SDK\OpenApi\Domain\ResultItem\Provider\OfficialDocumentationResultFieldProvider;
...
class OfficialDocumentationResultFieldProviderTest extends TestCase
{
    ...
    {
        $officialDocumentationResultFieldProvider = new OfficialDocumentationResultFieldProvider(
            new MockHttpClient(new MockResponse((string) file_get_contents(self::HTML_FIXTURE)))
        );

        $fields = $officialDocumentationResultFieldProvider->provide('https://apidocs.bitrix24.com/api-reference/chats/im-dialog-get.html', 'result-item');
