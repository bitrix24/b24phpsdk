...
class Batch extends \Bitrix24\SDK\Core\Batch
{
    ...
    public function getEntityItems(
        ...
        try {
            ...
            foreach ($this->getTraversableBatchResults(true) as $cnt => $getItemResult) {
                yield $cnt => $getItemResult;
            }
        } catch (InvalidArgumentException $exception) {
            $errorMessage = sprintf('batch get entity items: %s', $exception->getMessage());
            $this->logger->error(
                $errorMessage,
                [
                    'trace' => $exception->getTrace(),
        ...
        $this->logger->debug('getEntityItems.finish');
    }
