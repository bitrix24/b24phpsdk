...
namespace Bitrix24\SDK\Services\CRM\Item\Productrow;
 ...
 * Class Batch
 *
 * @package Bitrix24\SDK\Services\CRM\Item\Productrow
 */
            ...
            foreach ($entityItemId as $cnt => $code) {
                if (!is_int($code)) {
                    throw new InvalidArgumentException(
                        sprintf(
                            'invalid type «%s» of product row id «%s» at position %s, id must be integer type',
                            gettype($code),
                            $code,
                            $cnt
                        )
