...
{
    /**
     * Delete price type items with batch call using lowercase «id» parameter key
     *
     ...
     */
    public function deletePriceTypeItems(string $apiMethod, array $entityItemId): Generator
    {
        $this->logger->debug(
            'deletePriceTypeItems.start',
            [
                'apiMethod' => $apiMethod,
            ...
            foreach ($entityItemId as $cnt => $id) {
                if (!is_int($id)) {
                    throw new InvalidArgumentException(
                        sprintf(
                            'invalid type «%s» of price type id at position %s, id must be integer type',
                            gettype($id),
                            $cnt
                        )
                    );
        ...
        $this->logger->debug('deletePriceTypeItems.finish');
    }
}
