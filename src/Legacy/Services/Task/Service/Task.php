...
use Bitrix24\SDK\Legacy\Services\Task\Result\CountersResult;
use Bitrix24\SDK\Legacy\Services\Task\Result\DependenceResult;
use Bitrix24\SDK\Legacy\Services\Task\Result\HistoriesResult;
use Bitrix24\SDK\Legacy\Services\Task\Result\TaskFieldsResult;
use Bitrix24\SDK\Legacy\Services\Task\Result\TaskResult;
use Bitrix24\SDK\Legacy\Services\Task\Result\DeletedTaskResult;
use Bitrix24\SDK\Legacy\Services\Task\Result\UpdatedTaskResult;
use Bitrix24\SDK\Services\Task\Result\TasksResult;
use Psr\Log\LoggerInterface;

/**
 * @deprecated Use {@see \Bitrix24\SDK\Services\Task\Service\Task} via v3 API instead.
 *             This service exposes Bitrix24 REST API v1 task methods that have not yet
 *             been ported to v3. Will be removed once v3 reaches feature parity.
 */
...
class Task extends AbstractService
{
    ...
    public function add(array $fields): AddedTaskResult
    {
        return new AddedTaskResult(
            $this->core->call('tasks.task.add', ['fields' => $fields])
        );
    }
    ...
    public function delete(int $id): DeletedTaskResult
    {
        return new DeletedTaskResult(
            $this->core->call('tasks.task.delete', ['taskId' => $id])
        );
    }
    ...
    public function list(array $order = [], array $filter = [], array $select = [], int $start = 0, int $limit = 50): TasksResult
    {
        $params = $filter;
        $params['order'] = $order;
    ...
    public function update(int $id, array $fields): UpdatedTaskResult
    {
        return new UpdatedTaskResult(
            $this->core->call('tasks.task.update', ['taskId' => $id, 'fields' => $fields])
        );
    }
    ...
    {
        return new UpdatedTaskResult(
            $this->core->call('tasks.task.delegate', ['taskId' => $id, 'userId' => $userId])
        );
    }
    ...
    {
        return new UpdatedTaskResult(
            $this->core->call('tasks.task.start', ['taskId' => $id])
        );
    }
    ...
    {
        return new UpdatedTaskResult(
            $this->core->call('tasks.task.pause', ['taskId' => $id])
        );
    }
    ...
    {
        return new UpdatedTaskResult(
            $this->core->call('tasks.task.defer', ['taskId' => $id])
        );
    }
    ...
    {
        return new UpdatedTaskResult(
            $this->core->call('tasks.task.complete', ['taskId' => $id])
        );
    }
    ...
    {
        return new UpdatedTaskResult(
            $this->core->call('tasks.task.renew', ['taskId' => $id])
        );
    }
    ...
    {
        return new UpdatedTaskResult(
            $this->core->call('tasks.task.approve', ['taskId' => $id])
        );
    }
    ...
    {
        return new UpdatedTaskResult(
            $this->core->call('tasks.task.disapprove', ['taskId' => $id])
        );
    }
    ...
    {
        return new UpdatedTaskResult(
            $this->core->call('tasks.task.startwatch', ['taskId' => $id])
        );
    }
    ...
    {
        return new UpdatedTaskResult(
            $this->core->call('tasks.task.stopwatch', ['taskId' => $id])
        );
    }
    ...
    {
        return new UpdatedTaskResult(
            $this->core->call('tasks.task.mute', ['id' => $id])
        );
    }
    ...
    {
        return new UpdatedTaskResult(
            $this->core->call('tasks.task.unmute', ['id' => $id])
        );
    }
    ...
    public function addFavorite(int $id): UpdatedItemResult
    {
        return new UpdatedItemResult(
            $this->core->call('tasks.task.favorite.add', ['taskId' => $id])
        );
    }
    ...
    {
        return new UpdatedItemResult(
            $this->core->call('tasks.task.favorite.remove', ['taskId' => $id])
        );
    }
    ...
    public function getCounters(int $userId, int $groupId = 0, string $type = 'view_all'): CountersResult
    {
        return new CountersResult(
            $this->core->call('tasks.task.counters.get', [
                'userId'  => $userId,
                'groupId' => $groupId,
                'type'    => $type,
            ])
    ...
    public function getAccess(int $taskId, array $userIds = []): AccessesResult
    {
        return new AccessesResult(
            $this->core->call('tasks.task.getaccess', ['taskId' => $taskId, 'users' => $userIds])
        );
    }
    ...
    public function addDependence(int $taskIdFrom, int $taskIdTo, int $linkType): DependenceResult
    {
        return new DependenceResult(
            $this->core->call('task.dependence.add', [
    ...
    {
        return new DependenceResult(
            $this->core->call('task.dependence.delete', [
                'taskIdFrom' => $taskIdFrom,
                'taskIdTo'   => $taskIdTo,
            ])
        );
    ...
    public function historyList(int $id, int $start = 0): HistoriesResult
    {
        return new HistoriesResult(
            $this->core->call('tasks.task.history.list', ['taskId' => $id, 'start' => $start])
        );
    }
}
